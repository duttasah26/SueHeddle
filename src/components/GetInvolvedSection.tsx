"use client";

export default function GetInvolvedSection() {
  return (
    <div id="get-involved">
      {/* Row 1: Take Action */}
      <div className="involve-row-action">
        <div className="involve-row-inner">
          <p className="involve-section-label">Take action</p>
          <div className="action-grid">
            <a href="mailto:sue@sueheddle.ca?subject=Volunteer" className="action-btn">
              Sign up to Volunteer
            </a>
            <a href="mailto:sue@sueheddle.ca?subject=Lawn%20Sign%20Request" className="action-btn">
              Get a Sign
            </a>
            <a href="mailto:sue@sueheddle.ca?subject=Join%20Campaign" className="action-btn">
              Join our Campaign
            </a>
            <a href="/donate" className="action-btn">
              Make a Donation
            </a>
          </div>
        </div>
      </div>

      {/* Row 2: Connect with Sue */}
      <div className="involve-row-connect">
        <div className="involve-row-inner">
          <p className="involve-section-label">Connect with Sue</p>
          <div className="social-grid">
            <a href="mailto:sue@sueheddle.ca" className="social-btn social-btn--email">
              <span className="material-symbols-outlined">mail</span>
              Email
            </a>
            <a href="#" className="social-btn social-btn--twitter">
              <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true">
                <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.84 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
              </svg>
              Twitter
            </a>
            <a href="#" className="social-btn social-btn--instagram">
              <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
              </svg>
              Instagram
            </a>
            <a href="#" className="social-btn social-btn--bluesky">
              <span className="material-symbols-outlined">cloud</span>
              Bluesky
            </a>
          </div>
        </div>
      </div>

      {/* Row 3: Sign Up + Donate */}
      <div className="involve-row-signup">
        <div className="involve-row-inner">
          <div className="involve-signup-grid">
            <div>
              <h3 className="signup-form-title">Sign up for updates</h3>
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="signup-fields-row">
                  <input className="signup-input" type="text" placeholder="Name" />
                  <input className="signup-input" type="email" placeholder="Email" />
                </div>
                <div className="signup-fields-row">
                  <input className="signup-input" type="text" placeholder="Postal Code" />
                  <button type="submit" className="signup-submit">
                    Sign up
                    <span className="material-symbols-outlined">arrow_forward</span>
                  </button>
                </div>
              </form>
            </div>

            <div>
              <h3 className="donation-form-title">Make a donation</h3>
              <div className="donation-grid">
                <a href="/donate?amount=25" className="donation-btn">$25 →</a>
                <a href="/donate?amount=50" className="donation-btn">$50 →</a>
                <a href="/donate?amount=100" className="donation-btn">$100 →</a>
                <a href="/donate?amount=250" className="donation-btn">$250 →</a>
                <a href="/donate?amount=500" className="donation-btn">$500 →</a>
                <a href="/donate" className="donation-btn">Other →</a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
